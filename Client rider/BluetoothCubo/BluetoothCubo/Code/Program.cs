...
namespace BluetoothCubo
{
    internal class Program
    {
        ...
        private static SemaphoreSlim conectSemaphore;
        ...
        public static async Task Main(string[] args)
        {
            ...
            _jsonSaving = new JsonSaving(args[0]);
            _cubeTracker = new CubeTracker();
            conectSemaphore = new SemaphoreSlim(0);
...
//start Bluetooth 
        ...
        {
            namedPipeClient = new NamedPipeClientStream("connect");
            namedPipeClient.Connect();

            conectSemaphore.Wait();
            ChooseDevice();
            conectSemaphore.Release(1);
        }
        ...
        private static void ChooseDevice()
        {
            ...
            SendDataToServer(namedPipeClient, devices.Length.ToString());

            for (int i = 0; i < devices.Length; i++)
            {
                SendDataToServer(namedPipeClient, devices[i]);
                Console.WriteLine(i + " - " + devices[i]);
            }

            Console.WriteLine(" write the index of the device you want to connect to:");
            device = _devicesList.GetDevice(ReciveDataFromServer(namedPipeClient));
        }
